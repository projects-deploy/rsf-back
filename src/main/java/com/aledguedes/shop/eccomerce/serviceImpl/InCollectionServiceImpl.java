...
public class InCollectionServiceImpl implements InCollectionService {
    ...
    @Override
    public InCollectionResponse updateInCollection(InCollectionRequest inCollectionRequest, Long inCollection_id) {
        try {
            var newInCollection = inCollectionRepository.findById(inCollection_id)
                    .orElseThrow(ColecoesNotFoundException::new);
            BeanUtils.copyProperties(inCollectionRequest, newInCollection, "id", "createdAt", "updatedAt");
