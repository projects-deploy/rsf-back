...
public class InCollectionController {
    ...
    @ResponseStatus(HttpStatus.CREATED)
    public List<InCollectionResponse> saveInCollectionList(
            @RequestBody @Valid List<InCollectionRequest> inCollectionRequestList) {
        return inCollectionService.saveInCollectionList(inCollectionRequestList);
    }

    @PutMapping(value = "/{inCollection_id}")
    public InCollectionResponse updateInCollection(@RequestBody @Valid InCollectionRequest inCollection,
            @PathVariable Long inCollection_id) {
